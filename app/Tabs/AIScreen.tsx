...
import { getPersonalizedAISuggestions, formatPrice, formatRating, FashionItemForDisplay, trackUserInteraction, likeProduct, unlikeProduct, isProductLiked, getEnhancedPersonalizedAISuggestions, QuestionnairePreferences } from '../../backend/supabaseItems';
...
const AIScreen = () => {
  ...
  const [aiProducts, setAiProducts] = useState<FashionItemForDisplay[]>([]);
  ...
  useEffect(() => {
    if (user?.id && aiProducts.length === 0) {
      loadInitialAIProducts();
    }
  }, [user?.id]);
  ...
  useEffect(() => {
    if (user?.id && aiProducts.length > 0) {
      loadLikedStateForProducts(aiProducts.map(p => p.id));
    }
  }, [user?.id, aiProducts]);
  ...
  const loadInitialAIProducts = async () => {
    ...
    try {
      setLoading(true);
      console.log('Loading initial AI recommendations...');
      ...
      const questionnairePrefs: QuestionnairePreferences | null = preferences ? {
        ...
        fit: preferences.fit,
              ...
              console.log('AI using questionnaire preferences (AI SCREEN ONLY):', questionnairePrefs);
      
      // AI Screen uses questionnaire-based recommendations exclusively
      const products = await getEnhancedPersonalizedAISuggestions(user?.id || '', questionnairePrefs, 0, 10);
      
        setAiProducts(products);
        setPage(1);
      setHasMore(products.length > 0); // Set hasMore to true if any items returned
      console.log(`Loaded ${products.length} initial AI recommendations, hasMore: ${products.length > 0}`);
      
    } catch (error) {
      console.error('Error loading initial AI recommendations:', error);
    } finally {
      setLoading(false);
    }
  };

  const loadMoreAIProducts = async () => {
    if (loading || !hasMore) return;

    try {
      setLoading(true);
      console.log(`Loading more AI recommendations (page ${page})...`);
      ...
      const newProducts = await getEnhancedPersonalizedAISuggestions(user?.id || '', questionnairePrefs, page, 10);
      
      if (newProducts.length > 0) {
        setAiProducts(prev => [...prev, ...newProducts]);
        setPage(prev => prev + 1);
        console.log(`Loaded ${newProducts.length} more AI recommendations`);
        
        // Pre-create animations for new items with staggered delays
        newProducts.forEach((item, index) => {
          const itemKey = `ai-${item.id}-${aiProducts.length + index}`;
          createNewItemAnimation(itemKey, index * 50); // 50ms delay between each item
        });
      } else {
        setHasMore(false);
        console.log('📭 No more AI recommendations to load');
      }
    } catch (error) {
              console.error('Error loading more AI recommendations:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLikeToggle = async (productId: number, event?: any) => {
    ...
    try {
      ...
      if (isLiked) {
        ...
        const result = await likeProduct(user.id, productId, 'ai');
        if (result.success) {
  ...
  // Update filtered products when aiProducts changes or filters change
  useEffect(() => {
    if (aiProducts.length > 0) {
      if (hasActiveFilters()) {
        const filtered = applyFilters(aiProducts, activeFilters);
        setFilteredProducts(filtered);
      } else {
        setFilteredProducts(aiProducts);
      }
    }
  }, [aiProducts, activeFilters]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.titleSection}>
          <Text style={styles.pageTitle}>AI Recommendations</Text>
        </View>

        {/* Discover Section with Filter and Chat */}
        <View style={styles.discoverSection}>
          ...
          <View style={styles.rightIcons}>
            ...
            <TouchableOpacity style={styles.chatButton}>
              <Image source={require('../../assets/images/chatapi.png')} style={styles.chatIcon} />
              <Image source={require('../../assets/images/ss logo white.png')} style={styles.chatLogoOverlay} />
            </TouchableOpacity>
          </View>
        </View>
          ...
          >
            <View style={styles.productGrid}>
              {filteredProducts.map((item, index) => {
                if (index % 2 === 0) {
                  return (
                    <View key={`ai-row-${index}`} style={styles.productRow}>
